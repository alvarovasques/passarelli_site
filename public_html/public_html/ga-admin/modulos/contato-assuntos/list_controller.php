...
// ########## REMOCAO DE UM ITEM ##########
	if (isset($_GET["id"]) && $the_action == "Remove")
	{
		...
		if (!$rs  = _query($sql))
			add_debug_msg( _sqlError(), "contato-assuntos/list_controller.php" );
...
// labels
	if (!isset($the_result))
		$the_result = array('#', 'Título', 'E-Mail', 'Ações');
// a query 
	$sql = sprintf("SELECT `id`, `titulo`, `email`, `ativo`  FROM `%s` WHERE 1", $the_table);
// busca
	if ($the_search != "")
		$sql .= sprintf(" AND (`titulo` LIKE '%%%s%%' OR `email` LIKE '%%%s%%')", _escape($the_search), _escape($the_search));
// total de registros
...
/* Path: ga-admin/modulos/contato-assuntos/list_controller.php */