 ...
 *
 * @version   1.002, Created: 19/06/2013
...
// recupera e salva dados do POST (Edit, Add)
	if (isset($_POST["input"]))
	{
		...
		else
		{
			...
			if (!$rs  = _query($sql))
			{
				add_debug_msg( _sqlError(), sprintf("noticias/edit_controller.php[%s]", $the_action) );
...
// #################### MIDIAS CONFIG ####################
 // extensões permitidas para este módulo
	$allow_these = array( "jpg","png","gif","jpeg","ytb" );
 // ajuste para imagens e crop
	$widths  = array(320);
	$heights = array(240);
	$useCrop = "true";
    $uploadInterface = 'rbMidiaPack';
 // mensagem das midias e possíveis labels
	$midia_msg = "Resolução sugerida 800x600 pixels";
 // interface de upload (se necessa´rio)
	
/**
 * Inclusão do arquivo de controle das mídias, fazendo o save se vier mídias por "post"($_FILES),
 * ou dá acesso as funcionalidades de mídia para carregamentos de interfaces de upload...
 */
	include_once 'midia/midia_control.php';
...
/* Path: ga-admin/modulos/noticias/edit_controller.php */