...
// ########## REMOCAO DE UM ITEM ##########
	if (isset($_GET["id"]) && $the_action == "Remove")
	{
		...
		if (!$rs  = _query($sql))
			add_debug_msg( _sqlError(), "noticias/list_controller.php" );
...
// labels
	if (!isset($the_result))
		$the_result = array('#', 'Título', 'Autor', 'Ações');
// a query 
	$sql = sprintf("SELECT `id`, `titulo`, `autor` FROM `%s` WHERE 1", $the_table);
// busca
	if ($the_search != "")
		$sql .= sprintf(" AND (`titulo` LIKE '%%%s%%' OR `autor` LIKE '%%%s%%')", _escape($the_search), _escape($the_search));
// total de registros
...
/* Path: ga-admin/modulos/noticias/list_controller.php */