(function(w,$){
/* Tradução do nicEdit by Rafael Benites */
function _t(s){
	switch(s.toString()){
		case'Click to Bold':return"Negrito";break;
		case'Click to Italic':return'It&aacute;lico';break;
		case'Click to Underline':return'Sublinhado';break;
		case'Left Align':return'Alinhamento &agrave; Esquerda';break;
		case'Center Align':return'Centralizado';break;
		case'Right Align':return'Alinhamento &agrave; Direita';break;
		case'Justify Align':return'Justificado';break;
		case'Insert Ordered List':return'Numera&ccedil;&atilde;o';break;
		case'Insert Unordered List':return'Marcadores';break;
		case'Click to Subscript':return'Subescrito';break;
		case'Click to Superscript':return'Sobrescrito';break;
		case'Click to Strike Through':return'Riscado';break;
		case'Remove Formatting':return'Remover Formata&ccedil;&atilde;o';break;
		case'Indent Text':return'Espa&ccedil;amento';break;
		case'Remove Indent':return'Remover Espa&ccedil;amento';break;
		case'Horizontal Rule':return'Linha Horizontal';break;
		case'Select Font Size':return'Tamanho da Fonte';break;
		case'Select Font Family':return'Selecione uma Fonte';break;
		case'Select Font Format':return'Formato da Fonte';break;
		case'Add Link':return'Adicionar Link';break;
		case'Remove Link':return'Remover Link';break;
		case'Add/Edit Link':return'Inserir/Editar Link';break;
		case'Title':return'T&iacute;tulo';break;
		case'Open In':return'Destino';break;
		case'Current Window':return'Mesma Janela';break;
		case'New Window':return'Nova Janela';break;
		case'You must enter a URL to Create a Link':return'Insira uma url para criar um link!';break;
		case'Change Text Color':return'Trocar cor do texto';break;
		case'Change Background Color':return'Trocar cor de fundo do texto';break;
		case'Add Image':return'Inserir Imagem';break;
		case'Align':return'Posi&ccedil;&atilde;o';break;
		case'You must enter a Image URL to insert':return'Insira a url para inserir uma imagem!';break;
		case'Right':return'Direita';break;
		case'Left':return'Esquerda';break;
		case'Add/Edit Image':return'Inserir/Editar Imagem';break;
		case'Default':return'Padr&atilde;o';break;
		case'Alt Text':return'Alt';break;
		case'Save this content':return'Salvar conte&uacute;do';break;
		case'Edit HTML':return'Editar HTML';break;
	}
	return s;
}
...
var bkElement=bkClass.extend({
	...
	pos:function()
	{
		...
		A=(!window.opera)?parseInt(this.getStyle("border-width")||this.style.border)||0:0;
	...
	setStyle:function(A)
	{
		...
		for(var C in A)
		{
			switch(C)
			{
				case"float":B.cssFloat=B.styleFloat=A[C];break;
				case"opacity":B.opacity=A[C];B.filter="alpha(opacity="+Math.round(A[C]*100)+")";break;
				case"className":this.className=A[C];break;
				default:B[C]=A[C]
			}
		}
		return this
...
function $BK(A){
	...
	return(A&&!A.appendBefore)?bkExtend(A,bkElement.prototype):A
}
...
var nicEditorConfig=bkClass.extend({
	...
	iconsPath:'./includes/js/nicEdit/nicEditorIcons.gif',
...
var nicEditor=bkClass.extend({
	...
	panelInstance:function(B,C)
	{
		...
		A=new bkElement("DIV").setStyle({width:(parseInt(B.getStyle("width"))||B.clientWidth)+"px"}).appendBefore(B);
		this.setPanel(A);
	...
	addInstance:function(B,C)
	{
		...
		{
			A=new nicEditorInstance(B,C,this);
		}
		else
		{
			A=new nicEditorIFrameInstance(B,C,this);
		}
...
var nicEditorInstance=bkClass.extend({
	...
	{
		var A,B,E,F,H;
		H=(G.nodeName.toLowerCase()=="textarea");
		this.ne=C;
		this.elm=this.e=G;
		this.options=D||{};
		...
		{
			B=(bkLib.isMSIE&&!((typeof document.body.style.maxHeight!="undefined")&&document.compatMode=="CSS1Compat"));
			E={width:newX+"px",border:"1px solid #ccc",borderTop:0,overflowY:"auto",overflowX:"hidden"};
			...
			A=new bkElement("DIV").setStyle({width:(newX-8)+"px",margin:"4px",minHeight:newY+"px"}).addClass("main").appendTo(this.editorContain);
...
var nicEditorIFrameInstance=nicEditorInstance.extend({
	...
	init:function()
	{
		var A,B=this.elm.innerHTML.replace(/^\s+|\s+$/g,"");
		this.elm.innerHTML="";
		(!B)?B="<p><br /></p>":B;this.initialContent=B;
		this.elmFrame=new bkElement("iframe").setAttributes({src:"javascript:;",frameBorder:0,allowTransparency:"true",scrolling:"no"}).setStyle({height:"100px",width:"100%"}).addClass("frame").appendTo(this.elm);
		if(this.copyElm)
		{
			this.elmFrame.setStyle({width:(this.elm.offsetWidth-4)+"px"})
		}
		A=["font-size","font-family","font-weight","color"];
		for(var itm in A)
		{
	...
	initFrame:function()
	{
		var A,B=$BK(this.elmFrame.contentWindow.document);
		B.designMode="on";
		B.open();
		A=this.ne.options.externalCSS;
...
var nicEditorPanel=bkClass.extend({
	...
	reorder:function()
	{
		var A,C=this.buttonList;
		for(var B=0;B<C.length;B++)
		{
			A=this.findButton(C[B]);
			if(A)
			{
...
var nicEditorButton=bkClass.extend({
	construct:function(D,A,C,B){
		...
		B.addEvent("selected",this.enable.closure(this)).addEvent("blur",this.disable.closure(this)).addEvent("key",this.key.closure(this));this.disable();
		this.init()
	},
	init:function(){},
	hide:function(){
	...
	setBg:function(A)
	{
		...
		switch(A)
		{
			case"hover":B={border:"1px solid #666",backgroundColor:"#ddd"};break;
			case"active":B={border:"1px solid #666",backgroundColor:"#ccc"};break;
			default:B={border:"1px solid #efefef",backgroundColor:"#efefef"}
		}
...
var nicEditorFontFamilySelect=nicEditorSelect.extend({
	sel:{
		arial:"Arial","comic sans ms":"Comic Sans","courier new":"Courier New",
		georgia:"Georgia",
		helvetica:"Helvetica",
		impact:"Impact","times new roman":"Times","trebuchet ms":"Trebuchet",
		verdana:"Verdana"
	},
	init:function()
	{
...
var nicEditorBgColorButton=nicEditorColorButton.extend({
	colorSelect:function(A)
	{
		this.ne.nicCommand(((bkLib.isMSIE)?'back':'hilite') +'Color',A);
		this.removePane()
	}
});
...
var nicImageButton=nicEditorAdvancedButton.extend({
	...
	submit:function(B)
	{
		...
		if(this.im)
		{
			this.im.setAttributes({src:this.inputs.src.value,alt:this.inputs.alt.value});
			this.im.setStyle({'float':this.inputs.align.value});
		}
	}
});
...
var nicXHTML = bkClass.extend({
	...
	sizes : {1 : '8pt', 2 : '10pt', 3 : '12pt', 4 : '14pt', 5 : '18pt', 6 : '24pt'},	
	construct : function(nE)
	{
		this.ne = nE;
		if(this.ne.options.xhtml) {
			nE.addEvent('get',this.cleanup.closure(this));
		}
	},	
	cleanup : function(nI)
	{
		...
		var aux, xhtml = this.toXHTML(node);
	/* chrome fix */
		xhtml = xhtml.replace('<div', '<p');
		xhtml = xhtml.replace('</div', '</p');
	...
	// inicializações
		var rV = '',
			...
			nC = n.hasChildNodes && n.hasChildNodes(),
			eN = new Array();
		...
		switch(nT)
		{
			case 1:
				...
				if(r)
				{
					...
					{
						var at = nA[i],
							aN = at.nodeName.toLowerCase(),
							aV = at.nodeValue || n.style.cssText,
							cn = false; // forçar um continue 
						// povoar cn problemas no ie 
							cn = typeof(aV) == "function" ||(bkLib.isMSIE && aV && aV.indexOf && aV.indexOf('function(')!=-1);

						if( !at.specified || !aV || bkLib.inArray(this.stripAttributes,aN) || cn )
						{
							continue;
						}						
						switch(aN)
						{
							case 'style':
								...
								var css = aV.replace(/ /g,"");
								for(var itm in this.cssReplace) {
									if( css.indexOf(itm) != -1 ) {
										eN.push(this.cssReplace[itm]);
										css = css.replace(itm,'');
									}
								}
								cT += css;
							...
							break;
							case 'class':
								aV = aV.replace("Apple-style-span","");
							break;
							case 'size':
								cT += "font-size:"+this.sizes[aV]+';';
								aV = "";
							break;
							/* by Rafael Benites */
							case 'color':
						...
						if(aV)
						{
							aT += ' '+aN.toLowerCase()+'="'+aV+'"';
						}
					}
					if(cT)
					{
						aT += ' style="'+cT.toLowerCase()+'"';
					}
				...
				if( !nC && !bkLib.inArray(this.noShort,aN) )
				{
					if(r)
					{
						rV += ' />';
				...
				else
				{
					...
					{
						var j = this.toXHTML(n.childNodes[i],true,true);
						if(j)
						{
							rV += j;
						}
...
// registra as variáveis globais (bkLib, nicEditors, nicEditor )
	w.bkLib = bkLib;
	w.nicEditors = nicEditors;
	w.nicEditor = nicEditor;
})(window,jQuery);