...
var languageCode = 'pt-br';	// Possible values: en,no,nl,es,pt-br,da,it,sv,cz
...
var pathToImages = 'includes/js/images/';	// Relative to your HTML file
...
function changeSelectBoxYear(e,inputObj)
{
	...
	var startYear,
		yearItems = inputObj.parentNode.getElementsByTagName('DIV');
...
function changeSelectBoxHour(e,inputObj)
{
	...
	var startHour,
		hourItems = inputObj.parentNode.getElementsByTagName('DIV');
	if(inputObj.innerHTML.indexOf('-')>=0){
...
function slideCalendarSelectBox()
{
	...
	setTimeout(function(){ slideCalendarSelectBox(); },speedOfSelectBoxSliding);
...
function writeCalendarContent()
{
	...
	for(var o=0;o<dayArray.length;o++){
		cell = row.insertCell(-1);
		cell.innerHTML = dayArray[o];
	}
   ...
   if (showWeekNumber) {
	...
	for(var n=0;n<dayStartOfMonth;n++){
		cell = row.insertCell(-1);
		cell.innerHTML = '&nbsp;';
	}
	...
	if(iframeObj){
		if(!calendarContentDivExists){
			setTimeout(function(){ resizeIframe(); },350);
		}else{
			setTimeout(function(){ resizeIframe(); },10);
		}
	}
	   ...
	   if(!inputField.value.match(/^[0-9]*?$/gi)){
			...
			for(var i=0;i<elements.length;i++) {
				if(positionArray[elements[i]]>=0) {
					eval( ''+properties[i]+'= '+(inputField.value.substr(positionArray[elements[i]],propertyLength[i])/1)+';');
				}
			}
...
w.displayCalendar = displayCalendar;
w.slideCalendarSelectBox = slideCalendarSelectBox;
